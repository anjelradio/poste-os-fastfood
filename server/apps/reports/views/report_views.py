...
class ReportView(APIView):
    ...
    def get(self, request, report_type=None, report_format="pdf"):
        if report_type == "purchases":
            if report_format == "summary":
                return self._handle_purchases_summary(request)
            return self._handle_purchases(request, report_format)
        if report_type == "profits" and report_format == "summary":
            return self._handle_profit_summary(request)
        if report_type == "profits":
            return self._handle_profits(request, report_format)
        if report_type == "product-sales":
            if report_format == "summary":
                return self._handle_product_sales_summary(request)
    ...
    def _handle_purchases_summary(self, request):
        from_date = request.query_params.get("from_date")
        to_date = request.query_params.get("to_date")
        ...
        summary_result = build_purchases_summary_response(
            request,
            from_date=from_date_parsed,
            to_date=to_date_parsed,
            source=source,
        ...
        if summary_result.errors:
            return Response({"errors": summary_result.errors}, status=summary_result.status_code)
        return Response(summary_result.data, status=status.HTTP_200_OK)

    def _handle_profit_summary(self, request):
        from_date = request.query_params.get("from_date")
        to_date = request.query_params.get("to_date")
        source = request.query_params.get("source", "manual")
        from_date_parsed, to_date_parsed, date_error = self._validate_date_range(from_date, to_date)
        ...
        summary_result = build_profit_summary_response(
            request,
            from_date=from_date_parsed,
            to_date=to_date_parsed,
            source=source,
    ...
    def _handle_product_sales_summary(self, request):
        from_date = request.query_params.get("from_date")
        to_date = request.query_params.get("to_date")
        ...
        if not product_id:
            return Response(
                {"errors": ["El parÃ¡metro product_id es obligatorio"]},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        summary_result = build_product_sales_summary_response(
            request,
            from_date=from_date_parsed,
            to_date=to_date_parsed,
